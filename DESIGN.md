# Design notes: Microresistors

Reference design for `macros/resistors.lym`.

**Assumption throughout: every device is patterned from the same film**, so the resistivity $\rho$ and the thickness $t$ are common to all of them and the sheet resistance

$$R_s = \frac{\rho}{t} \qquad \left[\Omega \text{ per square}\right]$$

is a single constant for the whole mask.

## How length and width set the resistance

For a uniform bar of length $L$, width $W$ and thickness $t$:

$$R = \frac{\rho L}{W t} = \frac{\rho}{t} \cdot \frac{L}{W} = R_s N, \qquad N = \frac{L}{W}$$

$N$ is the **number of squares**: the count of $W \times W$ tiles laid end to end along the current path. It is dimensionless, and it is the only geometric quantity that survives.

- **Length is directly proportional.** Double $L$ and you double $R$ — twice as
  much material in series.
- **Width is inversely proportional.** Double $W$ and you halve $R$ — two identical paths in parallel.
- **Only the ratio matters.** $L = 100,\ W = 10$ and $L = 20,\ W = 2$ both give $N = 10$ and therefore the same resistance, despite the second occupying $25\times$ less area. Absolute size cancels.

Two effects break that ideal in practice, and the sweep is built to separate them:

- **Contact resistance** adds a constant, not a proportional, term: $R_\text{measured} = R_s N + 2 R_c$. It dominates when $N$ is small.
- **Linewidth bias.** Lithography and lift-off make the finished line differ
  from the drawn one by some $\Delta W$, so the effective count is $L / (W - \Delta W)$.
  A fixed $\Delta W$ is a rounding error at $W = 50$ and a large fraction at
...
A $20\times$ span in $N$ at constant width. Plotting $R$ against $L$ should give a
...
A $10\times$ span in width at constant length, giving the $1/W$ half of the relation.
...
Same square count, **$25\times$ less area**. Ideal-sheet theory says these two must
measure the same. If they do not, the difference is edge bias or film
non-uniformity, and the narrow one will be the one that is wrong — a $\Delta W$ of
...
$$R_s = \frac{\pi}{\ln 2} \cdot \frac{V}{I} \approx 4.532 \, \frac{V}{I}$$
...
A straight bar of $N \approx 48$ at $W = 10$ would be 480 µm long. Folding the same
...
A naive $\ell_\text{centreline} / W$ gives 52 squares. It over-counts, because current
...
one — 1.77 against 0.56, from the analytic annular-bend result

$$N_\text{corner} = \frac{\pi/2}{\ln\!\left(r_\text{out}/r_\text{in}\right)}, \qquad \frac{r_\text{out}}{r_\text{in}} = \frac{17}{7}$$

yet the device total is **lower**, because a bend of radius 12 µm consumes 24 µm
of centreline per corner against the sharp corner's 10 µm, and what it removes
from the straight runs outweighs what it adds back. In full, for $n$ corners:

$$N = \frac{\ell_\text{centreline} - n \, \ell_\text{consumed}}{W} + n \, N_\text{corner}$$
...
3. **$R$ vs $1/W$ at fixed $L$** — slope gives $R_s L$; curvature gives $\Delta W$.
